    ...
    try:
        ...
        for i in range(DYYf.size(0)): # For some reason, doing this in a loop doesn't crash
            try:
                ...
                '''
                    note-0108, fix a bug here torch.solve => torch.linalg.solve and change
                    DXYf:  torch.Size([1, 6, 23419110])
                    DYYf:  torch.Size([1, 6, 6])
                    Dy_at_x:  torch.Size([1, 6, 23419110])
                '''
                # print("DXYf: ", DXYf.size())
                # print("DYYf: ", DYYf.size())
                # print("Dy_at_x: ", Dy_at_x.size())
                # Dy_at_x[i, ...], _ = torch.linalg.solve(-1.0 * DXYf[i, ...], DYYf[i, ...])
                Dy_at_x[i, ...], _ = torch.linalg.solve(DYYf[i, ...], -1.0 * DXYf[i, ...], )
