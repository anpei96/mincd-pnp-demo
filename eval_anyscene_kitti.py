...
def eval_one_epoch(args, cfg, logger):
    ...
    for scene_dir in scene_dirs:
        ...
        for file_name in file_names:
            ...
            # print("img_corr_points: ", img_corr_points.shape)
            # print("pcd_corr_points: ", pcd_corr_points.shape)
            # import open3d as o3d
            # pcd_img = o3d.geometry.PointCloud()
            # pcd_img.points = o3d.utility.Vector3dVector(img_corr_points[:,:3])
            # colors = np.zeros_like(img_corr_points)
            # colors[:,0] = 1
            # pcd_img.colors = o3d.utility.Vector3dVector(colors[:,:3])
            # pcd_pcd = o3d.geometry.PointCloud()
            # pcd_pcd.points = o3d.utility.Vector3dVector(pcd_corr_points[:,:3])
            # colors = np.zeros_like(pcd_corr_points)
            # colors[:,1] = 1
            # pcd_pcd.colors = o3d.utility.Vector3dVector(colors[:,:3])
            # show_pcd(pcd_img+pcd_pcd)
            # assert 1==-1
