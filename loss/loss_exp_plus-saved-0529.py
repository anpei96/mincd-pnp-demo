...
class OverallLoss(nn.Module):
    def __init__(self, cfg):
        ...
        self.I_mat = torch.eye(3).cuda()
        self.T_mat = torch.eye(4).cuda()
    ...
    def forward(self, data_dict, output_dict):
        ...
        '''
            supervise topological relationships
        '''
        # loss of predicted topological relations
        gt_mat = data_dict["mat_primitive_gt"]
        pd_mat = output_dict["mat_primitive"]
        m, n = gt_mat.size()
        true_mask = (gt_mat > 0.5)
        loss_mat = 0.064*torch.mean(torch.norm(gt_mat[true_mask] - pd_mat[true_mask]))
        
        # loss of predicted pose
        if output_dict['wbpnp_status'] == 1:
            gt_tf = data_dict["transform"].detach()
            pd_tf = output_dict['est_transform']
            dR    = torch.matmul(gt_tf[:3,:3].t(), pd_tf[:3,:3])
            loss_t = 0.10*torch.norm(gt_tf[:3,3] - pd_tf[:3,3])
            loss_r = 0.10*torch.norm(dR - self.I_mat)
            loss = loss + loss_mat + loss_t + loss_r
        else:
            loss = loss + loss_mat

        '''
            topology reason prediction analysis --- ok, it actually works
        '''
        is_need_vis_analysis = True
        if is_need_vis_analysis:
            pred_mask = (pd_mat > 0.5)
            num_all = torch.sum(true_mask)
            num_cor = torch.sum(pred_mask & true_mask)
            pred_mask = (pd_mat > 0.05)
            num_cor_r = torch.sum(pred_mask & true_mask)
            print("loss_mat: ", loss_mat, "pre: ", num_cor/num_all, "recall: ", num_cor_r/num_all)
            if output_dict['wbpnp_status'] == 1:
                print("loss_t: ", loss_t, " loss_r: ", loss_r)
                '''
                    check whether it is more accurate than prior pose
                '''
                ini_tf = self.T_mat * 1.0
                dR    = torch.matmul(gt_tf[:3,:3].t(), ini_tf[:3,:3])
                loss_t = 0.10*torch.norm(gt_tf[:3,3] - ini_tf[:3,3])
                loss_r = 0.10*torch.norm(dR - self.I_mat)
                print("(before) loss_t: ", loss_t, " loss_r: ", loss_r)
            # print("gt_tf")
            # print(gt_tf)
            # print("pd_tf")
            # print(pd_tf)
