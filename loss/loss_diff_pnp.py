    ...
    def forward(self, data_dict, output_dict):
        ...
        '''
            ...
            in this scheme, the proposed scehme serves as a multi-task
                learning mechnisim
        '''
        pd_kpts = output_dict["coarse_kpts"]
        gt_kpts = data_dict["points_mask"]
        kpts_loss = torch.mean((pd_kpts[:,0] - gt_kpts[:,0])**2) + \
            torch.mean((pd_kpts[:,0] - gt_kpts[:,1])**2)
        ...
        if data_dict["epoch"] > 20: 
            dR = output_dict["dR"]
            dT = output_dict["dT"]
            pose_loss = (torch.norm(dR) + torch.norm(dT))/3.0*0.01

            pix_pd = output_dict["kpts_3d_pix_render"].float()
            pix_gt = output_dict["kpts_2d_pix"].float().detach()
            chamber_loss = self.chamfer_loss(pix_pd, pix_gt)*1e-2

        loss = loss + kpts_loss + pose_loss + chamber_loss

        return {"loss": loss, "c_loss": c_loss, "f_loss": f_loss, \
            "kpts_loss": kpts_loss, "pose_loss": pose_loss, \
            "chamber_loss": chamber_loss, "f_recall": f_recall}
