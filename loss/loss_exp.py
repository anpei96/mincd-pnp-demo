...
class OverallLoss(nn.Module):
    ...
    def forward(self, data_dict, output_dict):
        ...
        '''
            supervise mat_primitive with the loss in blind-pnp and go-match
        '''
        gt_mat = data_dict["mat_primitive_gt"]
        pd_mat = output_dict["mat_primitive"]
        true_mask = (gt_mat > 0)
        num_true_mask = torch.sum(true_mask)
        # loss_mat = torch.mean(-pd_mat[true_mask].log()) * 0.1
        loss_mat = torch.norm(gt_mat[true_mask] - pd_mat[true_mask])/num_true_mask
        loss = loss + loss_mat
        ...
        if is_need_vis_analysis:
            num_all = torch.sum(pd_mat)
            num_cor = torch.sum(gt_mat[true_mask] > 0.75)
            print("loss_mat: ", loss_mat, "top. recall: ", num_cor/num_all, "num_all: ", num_all, "num_cor: ", num_cor)
