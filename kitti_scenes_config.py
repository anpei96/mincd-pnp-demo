...
# exp
_C.exp = edict()
_C.exp.name = "output_kitti_scenes"
_C.exp.working_dir = osp.dirname(osp.realpath(__file__))
_C.exp.output_dir = osp.join("/media/anpei/DiskA/05_i2p_fewshot/outputs/", _C.exp.name)
...
# data
_C.data = edict()
_C.data.dataset_dir = "/media/anpei/DiskA/05_i2p_fewshot/data/kitti/val_selection_cropped/"
...
_C.eval.acceptance_radius = 0.05 #0.05
_C.eval.inlier_ratio_threshold = 0.1
_C.eval.rmse_threshold = 0.05 #0.1
...
_C.model.image_backbone.input_dim = 3 # rgb
...
def main():
    ...
    args = parse_args()
    if args.link_output:
        os.symlink(cfg.output_dir, _C.exp.name)
